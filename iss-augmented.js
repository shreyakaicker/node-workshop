...
//different values for "address" for educational purposes :slightly_smiling_face:
...
prompt.get('userLocation', function(err, answer) {
    if (err) {
        ...
        request(placeUrl, function getUserLatLong(err, location, body) {
            if (err) {
        ...
        calcDist(userLatitude,issLatitude,userLongitude,issLongitude);
